...
- **„Necesar montaj" — panou lateral per proiect**: fiecare dosar de proiect din „Pe proiecte" are acum un panou care iese din marginea dreaptă (lângă coloana Montaj), cu mâner vertical + pin. Conține `p.montajNeeds = [{ id, text, qty, done }]` — lista cu ce trebuie dus la montaj (bifabil, cu cantitate). Se deschide la hover/click pe mâner, se ascunde singur la mouse-out după ~1s dacă nu e pin-uit. Pin-ul se ține în `localStorage` per dispozitiv (`montaj-pin-<projectId>`), nu în date — fiecare își fixează ce vrea. Butonul × închide și anulează pin-ul. Ca să nu fie tăiat de `overflow:hidden` de pe `.proj-cluster`, panoul e copil al `.proj-wrap` (acum `position:relative`).
