...
- **„Necesar montaj" — panou lateral per proiect**: fiecare dosar de proiect din „Pe proiecte" are acum un panou care iese din marginea dreaptă (lângă coloana Montaj), cu mâner vertical. Conține `p.montajNeeds = [{ id, text, qty, done, supplierId }]` — lista cu ce trebuie dus la montaj: bifabil, cu cantitate și **furnizor** (același dropdown ca la „Comenzi materiale", populat din furnizorii existenți). Se deschide la hover/click pe mâner, se ascunde singur la mouse-out după ~1s. **Pin global** (📌 din orice panou) — un singur comutator în `localStorage` (`montaj-pin-all`), per dispozitiv: pornit → toate panourile stau deschise. Ca să nu fie tăiat de `overflow:hidden` de pe `.proj-cluster`, panoul e copil al `.proj-wrap` (acum `position:relative`).
